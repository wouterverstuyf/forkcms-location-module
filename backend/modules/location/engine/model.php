...
class BackendLocationModel
{
	...
	public static function delete($id)
	{
		...
		$extraorganization = array(
			'id' => $item['extra_organization_id'],
			'module' => 'location',
			'type' => 'widget',
			'action' => 'organization'
		);
		...
		$db->delete('modules_extras', 'id = ? AND module = ? AND type = ? AND action = ?', array($extraorganization['id'], $extraorganization['module'], $extraorganization['type'], $extraorganization['action']));
	...
	public static function insert($item)
	{
		...
		$extraorganization = array(
			...
			'sequence' => $db->getVar(
				'SELECT MAX(i.sequence) + 1
				 FROM modules_extras AS i
				 WHERE i.module = ?', array('organization')
				)
			);
		if(is_null($extraorganization['sequence']))
		{
			$extraorganization['sequence'] = $db->getVar(
				'SELECT CEILING(MAX(i.sequence) / 1000) * 1000 FROM modules_extras AS i'
			);
		}

		// insert extra - location widget
		$item['extra_id'] = $db->insert('modules_extras', $extra);
		$extra['id'] = $item['extra_id'];

		// insert extra - organization widget
		$item['extra_organization_id'] = $db->insert('modules_extras', $extraorganization);
		$extraorganization['id'] = $item['extra_organization_id'];
		...
		// insert extra - organization widget
		$extraorganization['data'] = serialize(array(
			'id' => $item['id'],
			'extra_label' => SpoonFilter::ucfirst(BL::lbl('Organization', 'core')) . ': ' . $item['title'],
			'language' => $item['language'],
			'edit_url' => BackendModel::createURLForAction('edit') . '&id=' . $item['id'])
		);
		$db->update('modules_extras', $extraorganization, 'id = ? AND module = ? AND type = ? AND action = ?', array($extraorganization['id'], $extraorganization['module'], $extraorganization['type'], $extraorganization['action']));
	...
	public static function update($item)
	{
		...
		if(isset($item['extra_organization_id']))
		{
			// build extra
			$extraorganization = array(
				'id' => $item['extra_organization_id'],
